# 代码架构

本文档基于 [思路.md](思路.md) 中的游戏设计，将设计思路转化为可执行的代码架构说明。

## 1. 项目概述

| 维度 | 说明 |
|------|------|
| **游戏类型** | 捣蛋猪式载具建造 + 粘粘世界式节点连接 |
| **技术栈** | Unity 3D URP 渲染 + 2D 物理（Rigidbody2D、Collider2D） |
| **核心原则** | 游戏逻辑与渲染/UI 分离，逻辑放在独立目录 |

## 2. 目录结构

```
Assets/
├── Scripts/
│   ├── GameLogic/           # 游戏逻辑（独立目录，与渲染/UI 解耦）
│   │   ├── Core/            # 核心抽象（IButtonReceiver、常量等）
│   │   ├── Nodes/           # 节点与连接
│   │   ├── Physics/         # 弹簧、力矩等物理计算
│   │   ├── Save/            # 存档
│   │   └── Level/           # 关卡状态机
│   ├── Rendering/           # 渲染相关（URP、材质、形状）
│   └── UI/                  # UI 组件与界面
├── Scenes/                  # 选关场景 + 各关卡场景
└── Documents/               # 思路.md、代码架构.md
```

## 3. 核心模块设计

| 模块 | 职责 | 关键类/接口 |
|------|------|-------------|
| **UI 抽象** | 按钮与接收者解耦 | `Button`, `IButtonReceiver` |
| **节点系统** | 节点类型、连接逻辑、物理 | `Node`, `NodeConnection`, `IConnectionStrategy` |
| **物理** | 弹簧、阻尼、断裂、力矩 | `SpringForce`, `AngularSpring` |
| **存档** | 蓝图、通关列表 | `SaveData`, `BlueprintData` |
| **关卡** | 建造/运行/过关状态 | `LevelState`, `LevelManager` |
| **相机** | 质心跟随、动态缩放 | `RuntimeCameraController` |

## 4. 关键类与接口

### 4.1 UI 抽象

**IButtonReceiver**：

```csharp
public interface IButtonReceiver {
    bool OnButtonDown(string buttonName);
}
```

**Button**：公共字段 `ButtonName`, `ButtonReceiver`，点击时调用 `ButtonReceiver.OnButtonDown(ButtonName)`。

### 4.2 节点系统

**Node**：基类，子类实现不同节点类型。

- `MaxConnectRadius`：主动连接最大半径
- `MaxConnectNumber = 3`：主动连接最大数量
- `SpringK`：弹性系数（公共静态字段）
- `SpringBreakLength`：弹簧断裂长度（公共静态字段）
- `CanRotate`：是否参与旋转/力矩
- `CanCollide`：是否参与物理碰撞
- `CanCharge`：是否作为供电源（可充电）
- `CanConduct`：是否可导电
- `IsCharged`：是否被网络供电（运行时状态）

**IConnectionStrategy**：抽象连接决策逻辑，便于后续调整。

```csharp
public interface IConnectionStrategy {
    void OnNodePlaced(Node node, IReadOnlyList<Node> candidates);
    void OnNodeDragged(Node node);
    IReadOnlyList<NodeConnection> GetPreviewConnections(Node node);
}
```

**NodeConnection**：表示两个节点之间的连接，建造阶段存储关系，运行阶段施加力；当两端都带电时可被渲染为绿色连接。

### 4.3 物理

- **径向力**：弹簧力 + 正比于速度的阻尼
- **切向力**：由两端力矩决定，满足角动量守恒
- **角度记录**：会旋转的节点记录每个连接的初始角度和上一帧角度，用于角向弹簧计算
- **运行切换对齐物理帧**：`LevelManager.EnterRunMode()` 通过 `WaitForFixedUpdate` 在物理步边界执行 Run 切换，降低首帧时序噪声
- **固定关键脚本顺序**：`ConnectionManager` 与 `TrianglePusher` 使用 `DefaultExecutionOrder` 固定 `FixedUpdate` 先后，避免默认调度导致的微小差异放大
- **地形运行时增量更新**：`AutoGround` / `AutoArcGround` 在编辑器中保持实时预览，在 Play 模式仅在端点或参数变化时才重建碰撞体

### 4.4 存档

**SaveData**：

- `CompletedLevelList`：已通过的关卡列表

**BlueprintData**：

- `List<NodeData> nodes`：所有节点（包括仓库中的和建造区域中的）
- `List<ConnectionData> connections`：连接关系
- `DeepCopy()`：深拷贝
- `DeleteBlueprint(int levelIndex)`：删除磁盘存档

**NodeData**：

- `GameObject prefab`：节点 Prefab 引用（用于实例化，不会序列化到磁盘）
- `string nodeType`：节点类型名（用于磁盘序列化和 prefab 查找）
- `bool isInInventory`：是否在仓库中
- `float posX, posY`：位置（仓库节点位置为 0，恢复时自动排列）

**LevelManager.InitialBlueprint**：

- 关卡设计师在 Inspector 中配置的初始蓝图
- 所有节点默认 `isInInventory = true`，只需设置 `prefab` 字段
- 进入关卡时若无磁盘存档，则使用 `InitialBlueprint` 的深拷贝
- 按 R 键可重置为初始蓝图

## 5. 数据流与场景

### 5.1 选关场景

- UI 预置在 Scene 中
- 读取 `SaveData.CompletedLevelList` 决定按钮颜色（绿/蓝/灰）与可点击性
- 解锁数量 = `CompletedLevelList.Length + InitialUnlockedLevelNum`

### 5.2 关卡场景

- `LevelManager` 进入时生成 UI（左上退出、右下开始/中止/下一关）
- 管理建造 / 运行 / 过关三种状态切换
- 启动流程：尝试从磁盘加载蓝图，若无则使用 `InitialBlueprint` 的深拷贝
- 蓝图包含所有节点（仓库 + 建造区域），离开关卡时保存到内存 + 硬盘
- 按 R 键删除磁盘存档并重置为初始蓝图

### 5.3 数据流图

```mermaid
flowchart TB
    subgraph LevelSelect [选关场景]
        SaveData[(SaveData)]
        LevelButtons[关卡按钮]
        SaveData --> LevelButtons
    end

    subgraph LevelScene [关卡场景]
        LevelManager[LevelManager]
        BlueprintMem[内存蓝图]
        BlueprintDisk[(硬盘蓝图)]
        LevelManager --> BlueprintMem
        LevelManager --> BlueprintDisk
        BlueprintMem --> LevelManager
    end

    LevelSelect -->|进入关卡| LevelScene
    LevelScene -->|退出/过关| LevelSelect
    LevelScene -->|过关时| SaveData
```

## 6. 物理与渲染分工

| 层级 | 职责 | 不负责 |
|------|------|--------|
| **Scripts/GameLogic** | 纯数据与逻辑（节点位置、连接关系、弹簧力计算、状态机） | 不直接操作 Mesh/Sprite |
| **Scripts/Rendering** | 根据 GameLogic 数据驱动 Mesh/Sprite 显示 | 不参与决策 |
| **物理** | 使用 Unity 2D 物理（Rigidbody2D、Collider2D），弹簧/连接力在 `FixedUpdate` 中施加 | - |

## 7. 常量与配置

| 常量 | 值 | 说明 |
|------|-----|------|
| `TotalLevelNum` | 10 | 总关卡数 |
| `InitialUnlockedLevelNum` | 3 | 初始解锁关卡数 |
| `MaxConnectNumber` | 3 | 节点主动连接最大数量 |
| `MaxHorizontalSpan` | 0.4 | 运行时相机水平方向节点到中心最大屏幕占比 |
| `MaxVerticalSpan` | 0.4 | 运行时相机竖直方向节点到中心最大屏幕占比 |

建议集中到 `GameConstants` 静态类或 ScriptableObject。

## 8. 类图

### 8.1 核心类关系

```mermaid
classDiagram
    class IButtonReceiver {
        <<interface>>
        +OnButtonDown(buttonName string) bool
    }

    class Button {
        +ButtonName string
        +ButtonReceiver IButtonReceiver
    }

    class IConnectionStrategy {
        <<interface>>
        +OnNodePlaced(node, candidates)
        +OnNodeDragged(node)
        +GetPreviewConnections(node)
    }

    class Node {
        +MaxConnectRadius float
        +MaxConnectNumber int
        +SpringK float
        +SpringBreakLength float
        +CanRotate bool
        +CanCollide bool
    }

    class NodeConnection {
        +NodeA Node
        +NodeB Node
        +RestLength float
        +InitialAngle float
    }

    class LevelManager {
        +LevelState state
        +EnterBuildMode()
        +EnterRunMode()
        +EnterVictoryMode()
    }

    class SaveData {
        +CompletedLevelList int[]
    }

    class BlueprintData {
        +Nodes NodeData[]
        +Connections ConnectionData[]
    }

    Button --> IButtonReceiver : uses
    Node --> IConnectionStrategy : uses
    Node --> NodeConnection : has many
    LevelManager --> SaveData : reads/writes
    LevelManager --> BlueprintData : reads/writes
```

### 8.2 关卡状态机

```mermaid
stateDiagram-v2
    [*] --> Build: 进入关卡
    Build --> Run: 点击开始
    Run --> Build: 点击中止
    Run --> Victory: 节点碰到终点
    Victory --> Build: 点击下一关(最后一关则退出)
    Victory --> [*]: 点击下一关(非最后一关)
    Build --> [*]: 点击退出
```

### 8.3 节点连接逻辑（建造阶段）

```mermaid
flowchart LR
    subgraph OnPlaced [节点放下时]
        A[找到 MaxConnectRadius 内] --> B[最近的 MaxConnectNumber 个]
        B --> C[未主动连接自己的节点]
        C --> D[对其主动连接]
    end

    subgraph OnDragged [节点拖动时]
        E[保留其它节点对它的主动连接] --> F{长度超 MaxConnectRadius 或 放入仓库?}
        F -->|是| G[断掉连接]
        F -->|否| H[保留]
    end

    subgraph Preview [拖动时]
        I[显示将形成的连接预览]
    end
```

## 9. 与思路.md 的对应关系

| 思路.md 章节 | 架构对应 |
|-------------|----------|
| 实现框架 | Scripts/GameLogic 独立目录、Button/IButtonReceiver |
| 选关页面 | LevelSelect 场景、SaveData、关卡按钮状态 |
| 关卡内部 | LevelManager、三种状态、UI 动态生成 |
| 节点 | Node、NodeConnection、IConnectionStrategy |
| 节点如何决定连接 | IConnectionStrategy、OnNodePlaced/OnNodeDragged |
| 节点和连接的物理特性 | SpringForce、AngularSpring、角度累积逻辑 |
| 运行时相机 | RuntimeCameraController、质心跟随、动态缩放 |
